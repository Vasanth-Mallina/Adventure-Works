...
product_lookup = product_lookup.toDF(*[col.replace(" ", "_").replace("-", "_") for col in product_lookup.columns])
product_category_lookup = product_category_lookup.toDF(*[col.replace(" ", "_").replace("-", "_") for col in product_category_lookup.columns])
product_subcategory_lookup = product_subcategory_lookup.toDF(*[col.replace(" ", "_").replace("-", "_") for col in product_subcategory_lookup.columns])
calendar_lookup = calendar_lookup.toDF(*[col.replace(" ", "_").replace("-", "_") for col in calendar_lookup.columns])
customer_lookup = customer_lookup.toDF(*[col.replace(" ", "_").replace("-", "_") for col in customer_lookup.columns])
return_data = return_data.toDF(*[col.replace(" ", "_").replace("-", "_") for col in return_data.columns])
territory_lookup = territory_lookup.toDF(*[col.replace(" ", "_").replace("-", "_") for col in territory_lookup.columns])
Sales_Data = Sales_Data.toDF(*[col.replace(" ", "_").replace("-", "_") for col in Sales_Data.columns])

# COMMAND ----------

product_lookup.write.option("header",'true').format('delta').save("/mnt/silver/Product_Lookup")
product_category_lookup.write.option("header",'true').format('delta').save("/mnt/silver/Product_Category_Lookup")
product_subcategory_lookup.write.option("header",'true').format('delta').save("/mnt/silver/Product_Subcategory_Lookup")
calendar_lookup.write.option("header",'true').format('delta').save("/mnt/silver/Calendar_Lookup")
customer_lookup.write.option("header",'true').format('delta').save("/mnt/silver/Customer_Lookup")
return_data.write.option("header",'true').format('delta').save("/mnt/silver/Returns_Data")
territory_lookup.write.option("header",'true').format('delta').save("/mnt/silver/Territory_Lookup")
Sales_Data.write.option("header",'true').format('delta').save("/mnt/silver/Sales_Data")
